...
function SmartAI:getValuableCardForGuanxing(cards)
	...
	for _, card in ipairs(cards) do
		for _, enemy in ipairs(new_enemies) do
			if not enemy:isNude() and isCard("Snatch", card, self.player) and self:hasTrickEffectiveSafe("snatch", card, enemy) and self.player:distanceTo(enemy) == 1 then
				snatch = card
				hasTrick = true
			elseif not enemy:isNude() and ((isCard("Dismantlement", card, self.player) and self:hasTrickEffectiveSafe("dismantlement", card, enemy))
											or (card:isBlack() and self.player:hasSkill("yinling") and self.player:getPile("brocade"):length() < 4)) then
				dismantlement = card
				hasTrick = true
			elseif isCard("Indulgence", card, self.player) and self:hasTrickEffectiveSafe("indulgence", card, enemy)
				and not enemy:containsTrick("indulgence") and not self:willSkipPlayPhase(enemy) then
				indulgence = card
				hasTrick = true
			elseif isCard("SupplyShortage", card, self.player) and self:hasTrickEffectiveSafe("supply_shortage", card, enemy)
				and not enemy:containsTrick("supply_shortage") and not self:willSkipDrawPhase(enemy) then
				supplyshortage = card
				hasTrick = true
			elseif isCard("Collateral", card, self.player) and self:hasTrickEffectiveSafe("collateral", card, enemy) and enemy:getWeapon() then
				collateral = card
				hasTrick = true
			elseif isCard("Duel", card, self.player) and (self:getCardsNum("Slash") >= getCardsNum("Slash", enemy, self.player) or self.player:getHandcardNum() > 4)
				and self:hasTrickEffectiveSafe("duel", card, enemy) then
				duel = card
				hasTrick = true
			...
			elseif isCard("FireAttack", card, self.player) and self:hasTrickEffectiveSafe("fire_attack", card, enemy)
			...
			elseif isCard("GodSalvation", card, self.player) and self:willUseGodSalvationSafe(card) then
				godsalvation = card
				hasTrick = true
	...
	for _, card in ipairs(cards) do
		for _, friend in ipairs(self.friends_noself) do
			if self:willSkipPlayPhase(friend, true) or self:willSkipDrawPhase(friend, true) or self:needToThrowArmor(friend) then
				if self:hasTrickEffectiveSafe("snatch", card, enemy) and isCard("Snatch", card, self.player) and self.player:distanceTo(friend) == 1 then
					snatch = card
					hasTrick = true
				elseif (isCard("Dismantlement", card, self.player) and self:hasTrickEffectiveSafe("dismantlement", card, enemy))
						or (card:isBlack() and self.player:hasSkill("yinling") and self.player:getPile("brocade"):length() < 4) then
					dismantlement = card
					hasTrick = true
				end
